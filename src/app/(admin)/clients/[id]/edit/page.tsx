...
export default function EditCustomerPage() {
  ...
  const [clientType, setClientType] = useState<'distributor' | 'private'>('distributor')
  ...
  useEffect(() => {
    ...
    setClientType(customer.client_type ?? 'distributor')
  ...
  const handleSave = async () => {
    ...
    const { error } = await supabase.from('customers').update({
      ...
      client_type: clientType, country, region: region || null, status,
    ...
    setSaving(false)
    if (!error) { queryClient.invalidateQueries({ queryKey: ['customers'] }); router.push('/clients') }
  ...
  return (
    <div className="max-w-4xl">
      <div className="flex items-center gap-4 mb-6">
        <Link href="/clients" className="text-gray-400 hover:text-gray-900"><ArrowLeft className="h-5 w-5" /></Link>
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-5">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              <select value={clientType} onChange={e => setClientType(e.target.value as 'distributor' | 'private')}
                className="mt-1 w-full h-9 rounded-md border border-gray-200 px-3 text-sm focus:outline-none">
