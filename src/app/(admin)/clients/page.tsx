...
export default function CustomersPage() {
  ...
  const filtered = customers.filter((c: any) => {
    ...
    const matchType = typeFilter === 'All' || (c.client_type ?? 'distributor') === typeFilter
    return matchSearch && matchStatus && matchCountry && matchType
  })
  ...
  const handleImport = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    alert(`Imported ${data.imported} clients!`)
    window.location.reload()
  }

  const exportCSV = () => {
    const headers = ['Legal Name', 'Trading Name', 'Type', 'Country', 'Price List', 'Status', 'VAT Number', 'Contact Name', 'Contact Email']
    const rows = filtered.map((c: any) => {
      const contact = c.contacts?.[0]
      return [
        c.legal_name, c.trading_name, c.client_type ?? 'distributor', c.country, c.assigned_price_list,
        c.status, c.vat_number,
        contact ? [contact.first_name ?? contact.name, contact.last_name].filter(Boolean).join(' ') : '',
        contact?.email ?? '',
      ]
    })
  ...
  const exportExcel = async () => {
    ...
    const rows = filtered.map((c: any) => {
      const contact = c.contacts?.[0]
      return {
        'Legal Name': c.legal_name, 'Trading Name': c.trading_name,
        'Type': c.client_type ?? 'distributor', 'Country': c.country,
        'Price List': c.assigned_price_list, 'Status': c.status,
        'VAT Number': c.vat_number,
        'Contact Name': contact ? [contact.first_name ?? contact.name, contact.last_name].filter(Boolean).join(' ') : '',
        'Contact Email': contact?.email ?? '',
      }
    })
    const ws = XLSX.utils.json_to_sheet(rows)
    ws['!cols'] = [25, 25, 12, 15, 10, 10, 18, 25, 25].map(w => ({ wch: w }))
    ...
    <div>
              ...
              {filtered.map((c: any) => {
                ...
                return (
                  <tr key={c.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-4 py-3">
                      <button onClick={() => router.push('/clients/' + c.id + '/edit')} className="text-gray-400 hover:text-gray-900 p-1 rounded hover:bg-gray-100">
      ...
      {selectedCustomer && (
        <DistributorPopup customer={selectedCustomer} onClose={() => setSelectedCustomer(null)}
          onEdit={() => { router.push('/clients/' + selectedCustomer.id + '/edit'); setSelectedCustomer(null) }} />
      )}
    </div>
  )
}
