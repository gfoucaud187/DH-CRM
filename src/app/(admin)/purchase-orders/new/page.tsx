...
export default function NewPurchaseOrderPage() {
  ...
  const [lines, setLines] = useState<Line[]>([
    { sku: '', description: '', quantity: 1, unit_price: 0 }
  ])
  ...
  const filteredPartners = (partners ?? []).filter((p: any) =>
    !poType || !p.category || p.category === poType
  )
  ...
  const handleSave = async (status: 'draft' | 'sent') => {
    ...
    // Generate PO number via function
  ...
  return (
    <div className="max-w-4xl">
      <div className="flex items-center gap-4 mb-6">
        <button onClick={() => setPoType(null)} className="text-gray-400 hover:text-gray-900">
          <ArrowLeft className="h-5 w-5" />
        </button>
      ...
      <div className="space-y-5">
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-5">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div className="col-span-2">
              ...
              {filteredPartners.length === 0 && partners && partners.length > 0 && (
                <p className="text-xs text-amber-600 mt-1">
                  No active partners with category "{poType}". You can still type a name manually.
                </p>
              )}
              {!partnerId && (
                <input
            ...
            <div>
              ...
              <div className="flex items-center gap-2 mt-1">
                ...
                <label className="flex items-center gap-1.5 text-sm text-gray-500 cursor-pointer whitespace-nowrap">
                  <input type="checkbox" checked={deliveryTba} onChange={e => setDeliveryTba(e.target.checked)}
                    className="rounded" />
                  TBA
                </label>
              </div>
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-5">
          ...
          <div className="space-y-2">
            <div className={`grid gap-2 text-xs font-medium text-gray-400 uppercase px-2 ${poType === 'cigars' ? 'grid-cols-[80px_1fr_80px_100px_80px_32px]' : 'grid-cols-[1fr_80px_100px_80px_32px]'}`}>
            ...
            {lines.map((line, i) => (
              <div key={i} className={`grid gap-2 items-center ${poType === 'cigars' ? 'grid-cols-[80px_1fr_80px_100px_80px_32px]' : 'grid-cols-[1fr_80px_100px_80px_32px]'}`}>
                {poType === 'cigars' && (
