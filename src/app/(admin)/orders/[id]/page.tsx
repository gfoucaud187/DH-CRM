...
import { ArrowLeft, Package, Truck, CheckCircle, XCircle, FileText, Clock, Edit, Send } from 'lucide-react'
    ...
    <div className="max-w-5xl">
      ...
      <div className="grid grid-cols-3 gap-6">
        <div className="col-span-1 space-y-4">
            ...
            <div className="bg-blue-50 rounded-xl border border-blue-200 p-4 space-y-2">
              ...
              {sourceDoc && (
                <button onClick={() => router.push(`/orders/${sourceDoc.id}`)}
                  ...
                  ← {sourceDoc.document_type.toUpperCase()}: {sourceDoc.order_number}
                </button>
              )}
              {linkedDoc && linkedDoc.document_type === 'invoice' && (
                <button onClick={() => router.push(`/orders/${linkedDoc.id}`)}
                  className="w-full text-left text-sm text-blue-700 hover:underline">
                  → INV: {linkedDoc.order_number}
                </button>
              )}
            </div>
          )}

          {/* Generate Invoice — only on SO without invoice */}
          {isSO && !alreadyHasInvoice && (
            <div className="bg-white rounded-xl border border-gray-200 p-4">
              <h2 className="font-semibold text-gray-900 mb-1">Generate Invoice</h2>
              <p className="text-xs text-gray-500 mb-3">
                Creates an invoice linked to {order.order_number}. The SO is kept as-is.
              </p>
          ...
          {/* Create FOC Document — only on SO, not already FOC */}
{isSO && !order.is_foc && (
  <div className="bg-white rounded-xl border border-gray-200 p-4">
    <h2 className="font-semibold text-gray-900 mb-1">Create FOC Document</h2>
    <p className="text-xs text-gray-500 mb-3">
      Creates a SO(DO) linked to {order.order_number} for free-of-charge items.
    </p>
    <button
      onClick={async () => {
      ...
      Create SO(DO)
              ...
              {statuses.map(s => (
                <button key={s.value} onClick={() => updateStatus(s.value)}
                  ...
                  className={`w-full flex items-center gap-2 px-3 py-2 rounded-lg text-sm transition-colors ${
                    s.value === order.status ? s.color + ' font-medium' : 'text-gray-600 hover:bg-gray-50'
                  }`}>
          ...
          {focOrder && (
            <div className="bg-green-50 rounded-xl border border-green-200 p-4">
              ...
              <p className="text-sm text-green-700 font-mono">{focOrder.order_number}</p>
        ...
        <div className="col-span-2 space-y-4">
          <div className="grid grid-cols-3 gap-3">
            {[
              ...
              { label: 'Total Amount', value: `${order.currency} ${Number(order.total_amount).toFixed(2)}` },
            ].map(({ label, value }) => (
