...
const CONTACT_TYPES = ['Distributor', 'Retailer', 'Agent', 'Direct']
...
export default function EditCustomerPage() {
  ...
  const [contactType, setContactType] = useState('Distributor')
  ...
  useEffect(() => {
    ...
    setContactType(customer.contact_type ?? 'Distributor')
  ...
  if (!customer) return <div className="text-center py-12 text-gray-400">Distributor not found</div>
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-5">
          ...
          <div className="flex flex-wrap gap-6">
            <label className="flex items-center gap-3 cursor-pointer">
              <input type="checkbox" checked={isEuropean} onChange={e => setIsEuropean(e.target.checked)}
                className="rounded" />
              <span className="text-sm font-medium text-gray-700">European client</span>
            </label>
            <label className="flex items-center gap-3 cursor-pointer">
              <input type="checkbox" checked={trackTrace} onChange={e => setTrackTrace(e.target.checked)}
                className="rounded" />
              ...
              <div className="flex gap-3 mt-2">
                {['TT', 'PR', ''].map(t => (
                  <label key={t} className="flex items-center gap-2 cursor-pointer">
                    <input type="radio" value={t} checked={euComplianceType === t}
                      onChange={() => setEuComplianceType(t)} />
